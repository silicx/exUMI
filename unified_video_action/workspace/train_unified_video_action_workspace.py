...
from unified_video_action.eval.eval import test_video_display, test_action_l2
    ...
    def run(self):
        ...
        if cfg.training.resume:
            ...
            if lastest_ckpt_path.is_file():
                ...
                self.load_checkpoint(path=lastest_ckpt_path, reset_lr=cfg.training.reset_lr)
        ...
        for local_epoch_idx in range(cfg.training.num_epochs):
                ...
                for batch_idx, batch in enumerate(tepoch):
                    ...
                    if (
                        ...
                        with torch.autocast(device_type="cuda", dtype=torch.bfloat16): # You might need to change the device_type to str(device) for other versions of torch
                            raw_loss, (loss_diffusion, loss_action, loss_vae, loss_mse) = self.model(batch)
                    else:
                        raw_loss, (loss_diffusion, loss_action, loss_vae, loss_mse) = self.model(batch)
                    ...
                    loss_vae_cpu = loss_vae.item()
                    loss_mse_cpu = loss_mse.item()

                    step_log = {
                        "train_loss": raw_loss_cpu,
                        "diffusion_loss": loss_diffusion_cpu,
                        "action_loss": loss_action_cpu,
                        "vae_loss": loss_vae_cpu,
                        "mse_loss": loss_mse_cpu,
                        "global_step": self.global_step,
                        "epoch": self.epoch,
                        "lr": self.lr_scheduler.get_last_lr()[0],
                    }
                    # print(loss_diffusion_cpu, loss_action_cpu, loss_vae_cpu, loss_mse_cpu)
            ...
            if (
                ...
                metric_dict = {"valid_mse_loss": 0.0}
