...
def is_nonactive_tactile(tactile: torch.Tensor) -> bool:
    value_threshold = 0.16
    ratio_threshold = 0.0005  # 25 pixels for 224*224

    assert tactile.ndim == 4 and tactile.shape[1]==3, str(tactile.shape)
    T, C, W, H = tactile.shape
    tactile = tactile[T//2:, 0:2, ...]  # take the concave and convex channels
    max_heatmap = tactile.max(dim=0).values.max(dim=0).values  # max activate map (224x224)

    activated_ratio = (max_heatmap > value_threshold).float().mean().item()
    # Check if the tactile data is non-active
    return activated_ratio > ratio_threshold


class UmiMultiTactileDataset(Dataset[batch_type]):
    """
    ...
    def __init__(
        ...
        obs_mask: str,
        nonactive_tactile_retry: int,
        nonactive_tactile_reject_rate: float,
        normalizer_type: str,
        **base_config: Union[dict[str, Any], DictConfig],
    ):
        self.nonactive_tactile_retry: int = nonactive_tactile_retry
        self.nonactive_tactile_reject_rate: float = nonactive_tactile_reject_rate
    ...
    def post_process(self, data_dict):
        # obs tactile_combined torch.Size([8, 3, 224, 224])
        # obs camera0_rgb torch.Size([1, 3, 224, 224])
        # obs robot0_eef_pos torch.Size([32, 3])
        # obs robot0_eef_rot_axis_angle torch.Size([32, 6])
        # obs robot0_eef_rot_axis_angle_wrt_start torch.Size([32, 6])
        # obs robot0_gripper_width torch.Size([32, 1])
        # obs img_indices torch.Size([8, 1])
        # action torch.Size([32, 10])
        # ids torch.Size([1])
        ...
        def exactly_like(like_generator, tensor):
            return like_generator(tensor, dtype=tensor.dtype, device=tensor.device)
        
        
        if self.obs_mask in ["action", "action_rgb"]:
            
            data_dict["action"] = exactly_like(torch.zeros_like, data_dict["action"])
            for key in [
                "robot0_eef_pos",
                ...
                data_dict["obs"][key] = exactly_like(torch.zeros_like, data_dict["obs"][key])
            
            
            
        if self.obs_mask in ["rgb", "action_rgb"]:
            data_dict["obs"]["camera0_rgb"] = exactly_like(torch.zeros_like, data_dict["obs"]["camera0_rgb"])
            # obs camera0_rgb torch.Size([1, 3, 224, 224])
            
            
        if self.obs_mask == "tactile":
            for key in data_dict["obs"]:
                if key.startswith("tactile_combined"):
                    half_size = data_dict["obs"][key].shape[0] // 2
                    data_dict["obs"][key][:half_size, ...] = \
                        exactly_like(torch.zeros_like, 
                                    data_dict["obs"][key][:half_size, ...])
    ...
    def __getitem__(self, idx: int) -> batch_type:
        ...
        while True:
            ...
            if not is_nonactive_tactile(data_dict['obs']['tactile_combined']):
                # good data
                data_dict["valid"] = torch.LongTensor([1])
                return self.post_process(data_dict)
            
            if retry >= self.nonactive_tactile_retry:
                # exceed the retry limit
                data_dict["valid"] = torch.LongTensor([0])
                return self.post_process(data_dict)
            
            if random.random() > self.nonactive_tactile_reject_rate:
                # lucy dog
                data_dict["valid"] = torch.LongTensor([0])
                return self.post_process(data_dict)
