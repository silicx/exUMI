...
class MAR(nn.Module):
    ...
    def __init__(
        ...
        **kwargs
        ...
        #     768, encoder_embed_dim, bias=True
        # )  # clip text embedding is 512
        self.text_input_proj = nn.Linear(768, encoder_embed_dim, bias=True)
        self.text_transformer = Block(
            encoder_embed_dim,
            encoder_num_heads,
            mlp_ratio,
            qkv_bias=True,
        ...
        self.text_proj_norm = norm_layer(encoder_embed_dim)
    ...
    def forward(
        ...
        if text_latents is not None:
            ...
            text_latents = self.text_input_proj(text_latents)
            text_latents = self.text_transformer(text_latents)
            text_latents = self.text_proj_norm(text_latents)
            text_latents = text_latents.mean(dim=1)
