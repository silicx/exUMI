...
class Camera:
    ...
    def get_rectify_image(self, raw_img):
        img = self.rectify_image(raw_img)
        return img

    def get_rectify_crop_image(self, raw_img):
        img = self.crop_image(self.get_rectify_image(raw_img))
        return img
...
class Sensor(Camera):
    def __init__(self, cfg):
        ...
        self.ref = cv2.imread(self.camera_calibration_dir + "/ref.png")
    ...
    def raw_image_2_xy_repr(self, img_GRAY, scale=7):
        diff_darker = self.ref_GRAY.astype(np.float32) - img_GRAY.astype(np.float32)
        diff_darker[diff_darker < 0] = 0
        diff_brighter = img_GRAY.astype(np.float32) - self.ref_GRAY.astype(np.float32)
        diff_brighter[diff_brighter < 0] = 0

        mixed_image = np.zeros_like(self.ref)
        mixed_image[::, ::, 0] = img_GRAY
        mixed_image[::, ::, 1] = diff_brighter * scale
        mixed_image[::, ::, 2] = diff_darker * scale
    ...
    def get_height_map(self, raw_img):
        img = self.get_rectify_crop_image(raw_img)
        img_GRAY = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
    ...
    def get_depth_map(self, raw_image):
        height_map = self.get_height_map(raw_image)
    ...
    def get_point_cloud(self, raw_img):
        height_map = self.get_height_map(raw_img)
        points = self.height_map_2_point_cloud(height_map)
        return points

    def get_point_cloud_gradients(self, raw_img):
        height_map = self.get_height_map(raw_img)
