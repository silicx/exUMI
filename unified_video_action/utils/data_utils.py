...
def resize_image(cfg, x):
    ...
    elif "tactile" in cfg.task.name:
        if "tactile_combined_view1" in x['obs']:
            # BYOL
            resize = 224
        
        for key in x['obs']:
            if key.startswith("tactile_combined"):
                B, T, C, H, W = x["obs"][key].shape
                resized_tensor = F.interpolate(
                    x["obs"][key].contiguous().float().view(B * T, C, H, W),
                    size=(resize, resize),
                    mode="bilinear",
                    align_corners=False,
                )
                x["obs"][key] = resized_tensor.view(
                    B, T, C, resize, resize
                )
...
def process_data(batch, task_name="", eval=False, main_key="tactile_combined", **kwargs):
    train = not eval

    x = batch["obs"][main_key]
    x = x * 255.0
    B, T, C, H, W = x.size()
    device = x.device
...
def get_trajectory(nactions, T, shift_action, use_history_action=False):
    if nactions is not None:
        assert nactions.shape[1] == T, f"nactions shape {nactions.shape} T {T}"
        if use_history_action:
            if shift_action:
                history_trajectory = nactions[:, : T // 2]
                trajectory = nactions[:, T // 2 : ]
            else:
...
def extract_latent_autoregressive(vae_model, x):
    ...
    return z, latent_size, posterior
...
def get_vae_latent(x, vae_model, eval=False, proprioception_input={}):
    ...
    if proprioception_input is not None:
        if "second_image" in proprioception_input:
            second_image_z, _, _ = extract_latent_autoregressive(
        ...
        if "pred_second_image" in proprioception_input:
            pred_second_image_z, _, _ = extract_latent_autoregressive(
                vae_model, proprioception_input["pred_second_image"]
            )
            proprioception_input["pred_second_image_z"] = pred_second_image_z

    if train:
        z, latent_size, z_posterior = extract_latent_autoregressive(vae_model, x)
        c, latent_size, _ = extract_latent_autoregressive(vae_model, c)
    else:
        with torch.no_grad():
            z, latent_size, z_posterior = extract_latent_autoregressive(vae_model, x)
            c, latent_size, _ = extract_latent_autoregressive(vae_model, c)

    return x, z, c, latent_size, proprioception_input, z_posterior
