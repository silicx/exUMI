...
class UmiLazyDataset(BaseLazyDataset):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        print("Num of views:", num_views)
        
        if self.num_views is not None:
            old_key = "tactile_combined"
            
            for i_view in range(1, self.num_views):
                new_key = old_key+"_view%d"%i_view

                self.transforms.transforms[new_key] = self.transforms.transforms[old_key]
                self.transforms.data_meta[new_key] = self.transforms.data_meta[old_key]
                if self.normalizer is not None:
                    self.normalizer.normalizers[new_key] = self.normalizer.normalizers[old_key]
    ...
    def _process_source_data(
        ...
        # tactile key
        # tactile_key = "tactile_combined"
        for key, value in data_dict.items():
            if "tactile" in key:
                assert not self.mask_mirror

                assert len(value.shape) == 4, str(value.shape)
                resized = []
                for i in range(value.shape[0]):
                    resized.append(
                        cv2.resize(value[i], (224, 224), interpolation=cv2.INTER_LINEAR)
                    )
                processed_data_dict[key] = np.stack(resized, axis=0)
    ...
    def __getitem__(self, idx: int):
        ...
        for entry_meta in self.source_data_meta.values():
            ...
            if (
                ...
                and "tactile" in entry_meta.name
            ):
            ...
            except imagecodecs._jpegxl.JpegxlError as e:
                print(self.name, entry_meta.name, global_indices)
                raise e
        ...
        for entry_meta in self.output_data_meta.values():
            ...
            if "tactile" in entry_meta.name and self.num_views is not None:
                for i_view in range(1, self.num_views):
                    new_key_name = entry_meta.name+"_view%d"%i_view
                    output_data_dict[entry_meta.usage][new_key_name] = copy.deepcopy(processed_data)
